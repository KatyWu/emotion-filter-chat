...
async function processText(text, filter, srcLang, translateTo) {
  ...
  if (!filter && translateTo) {
    const prompt = translateTo === "en"
      ? `把以下訊息翻譯成自然口語英文，保留語氣和斷句節奏，只輸出翻譯結果。訊息：`
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 500, messages: [{ role: "user", content: prompt + text }] }),
    });
    const data = await res.json();
    return { action: "send", text: data.content[0].text.trim(), filtered: false, translated: true };
  }

  let prompt = srcLang === "zh" ? CLASSIFY_PROMPT_ZH : CLASSIFY_PROMPT_EN;
  if (translateTo) {
    ...
    prompt = srcLang === "zh" ? prompt.replace("訊息：", note + "\n\n訊息：") : prompt.replace("Message: ", note + "\n\nMessage: ");
  }
